    ...
    function ModelSuitePage() {
      ...
      const saveModel = async (id) => {
        ...
        try {
          ...
          // O8：先校验数字输入——Number('abc')=NaN 会被 host 静默按"未填"处理，
          // 用户以为改成功了；这里就地报错，错误显示在该模型的操作条里。
          const cvRaw = String(ed0.contextWindow == null ? '' : ed0.contextWindow).trim()
          const mvRaw = String(ed0.maxTokens == null ? '' : ed0.maxTokens).trim()
          if (cvRaw && !/^\d+$/.test(cvRaw)) throw new Error(t('字段 ') + 'contextWindow' + t(' 需要整数'))
          if (mvRaw && !/^\d+$/.test(mvRaw)) throw new Error(t('字段 ') + 'maxTokens' + t(' 需要整数'))
