...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const api = {
      ...
      testModel: (args) => suiteFetch(API + '/test-model', args),
    ...
    const EN = {
      ...
      'off 勾选「空传」表示发送 null。': 'For off, ticking「send empty」means the parameter is sent as null.',
      ' 个开关。': ' fields.',
      '测试提示词': 'Test prompt',
    ...
    const EN_MSG_PATTERNS = [
      ...
      [/^已删除 (.+)，剩余 (\d+) 个模型$/, 'Deleted $1, $2 models remaining'],
      ...
      [/^目录命中 (\d+) 个，但无需补全（已有字段(，可开覆盖)?）$/, 'catalog matched $1, nothing to fill (fields present$2)'],
    ...
    const RESERVED_HEADER_NAMES = [
      ...
      'transfer-encoding', 'content-encoding', 'origin', 'referer', 'cookie',
    ];
    ...
    function ModelSuitePage() {
      ...
      /**
       * 用 bootstrap 的渠道行刷新「高级设置」四张卡片与「同步与自动化」草稿。
       ...
       */
      const applyProviderSnapshot = async (res, scope) => {
        ...
        if (fill) fill(next, providerRef.current || provider)
        return next
      }

      const loadDetail = React.useCallback(async (prov, bootNow) => {
        ...
        const map = {}
        for (const m of (res.models || [])) map[m.id] = toEditor(m, fields)
        setEditors(map)
        setModelErrors({})
      }, [])
      ...
      const switchProvider = async (next) => {
        ...
        providerEpochRef.current += 1
        const nextRow = rowOf(boot, next)
        const nextFields = compatFieldsFor(boot, nextRow && nextRow.api)
        ...
        setCompatDraft(compatDraftFromValue(nextFields, nextRow && nextRow.compat))
        try { await loadDetail(next, boot) } catch (e) { setError(e && e.message ? e.message : String(e)) } finally { setBusy(false) }
      }
      ...
      const patchCompat = (id, field, value) => setEditors((prev) => {
        const cur = prev[id] || { compat: {} }
        const compat = Object.assign({}, cur.compat || {})
        compat[field] = value
        return Object.assign({}, prev, { [id]: Object.assign({}, cur, { compat: compat }) })
      })
      ...
      const saveModel = async (id) => {
        ...
        try {
          ...
          const compat = compatPayloadFromDraft(fields, ed0.compat)
      ...
      const confirmAddRefreshedModels = async () => {
        ...
        try {
          ...
          closeRefreshPanel()
          await loadDetail(targetProvider, boot)
          setBoot(await api.bootstrap())
        } catch (e) {
          setError(e && e.message ? e.message : String(e))
        } finally {
      ...
      const runDiscoverModels = async () => {
        ...
        try {
          ...
          const res = await api.discoverModels({ baseURL: baseURL, api: apiName, apiKey: '' })
      ...
      const saveCompatCard = async () => {
        ...
        try {
          ...
          const warns = responseWarnings(res)
          if (warns.length) setWarnings(warns)
          await applyProviderSnapshot(res, 'compat')
        } catch (e) { setError(e && e.message ? e.message : String(e)) }
        finally { setAdvBusy('') }
      }
      ...
      const saveRetryCard = async () => {
        ...
        try {
          ...
          const warns = responseWarnings(res)
          if (warns.length) setWarnings(warns)
          await applyProviderSnapshot(res, 'retry')
        } catch (e) { setError(e && e.message ? e.message : String(e)) }
        finally { setAdvBusy('') }
      }
      ...
      const saveDefaultsCard = async () => {
        ...
        try {
          ...
          const warns = responseWarnings(res)
          if (warns.length) setWarnings(warns)
          await applyProviderSnapshot(res, 'defaults')
        } catch (e) { setError(e && e.message ? e.message : String(e)) }
        finally { setAdvBusy('') }
      }
      ...
      const validateHeaderRows = () => {
        ...
        if (serialized.length > HEADER_TOTAL_MAX) throw new Error(t('自定义请求头已超过 8 KB 上限'))
      ...
      const saveHeadersCard = async () => {
        ...
        try {
          ...
          setWarnings(warns)
          await applyProviderSnapshot(res, 'headers')
        } catch (e) { setError(e && e.message ? e.message : String(e)) }
        finally { setAdvBusy('') }
      }
        ...
        try {
          ...
          const payload = {
            sources: {
              ...
              litellm: { url: String(sd.litellm.url || '').trim(), enabled: !!sd.litellm.enabled },
              openrouter: { url: String(sd.openrouter.url || '').trim(), enabled: !!sd.openrouter.enabled },
            },
          }
      ...
      const renderAddPanel = () => {
        ...
        return React.createElement('div', { className: 'mp-prov mp-add-models' },
          ...
          React.createElement('div', { className: 'mp-actions' },
            ...
            }, addBusy ? t('提交中…') : (t('确认添加（') + pending.length + '）')),
          ),
        )
      }
                                  ...
                                  React.createElement('div', { className: 'mp-compat' },
                                    React.createElement('div', { className: 'mp-compat-head' },
                                      React.createElement('button', {
                                        ...
                                        + (ed.levels || []).filter((row) => row && row.enabled).length + t(' 项已配置') + '）'),
                                    ...
                                    ed.showWire && !ed.disabled && React.createElement(React.Fragment, null,
                                      React.createElement('div', { className: 'mp-ms-hint' }, t('off 勾选「空传」表示发送 null。')),
        ...
        return React.createElement('div', { className: 'mp-card' },
          ...
          React.createElement('div', { className: 'mp-about-grid' },
            ...
            React.createElement('div', { className: 'mp-about-row' },
              ...
              React.createElement('div', { className: 'mp-stack' },
                React.createElement('a', { className: 'mp-link', href: sd.modelsDev.url, target: '_blank', rel: 'noopener noreferrer' },
                  'models.dev · ' + sd.modelsDev.url),
                React.createElement('a', { className: 'mp-link', href: sd.litellm.url, target: '_blank', rel: 'noopener noreferrer' },
                  'LiteLLM · ' + sd.litellm.url),
                React.createElement('a', { className: 'mp-link', href: sd.openrouter.url, target: '_blank', rel: 'noopener noreferrer' },
                  'OpenRouter · ' + sd.openrouter.url),
              ),
            ),
          ),
