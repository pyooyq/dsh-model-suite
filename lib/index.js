...
function normalizeHeadersInput(raw) {
  ...
  for (const name of Object.keys(raw)) {
    ...
    if (RESERVED_HEADER_NAMES.indexOf(key.toLowerCase()) >= 0) {
...
function applyRemoteToLocal(localModel, remote, overwrite, syncInput) {
  let next = cloneModel(localModel) || { id: str(localModel && localModel.id, '') }
  const notes = []
  let changed = false
...
export function apply(ctx) {
  ...
  function modelView(m) {
    ...
    return {
      ...
      source: paramSource(m),
    }
  }

  /** 参数来源提示（§12.8 术语统一）。基于已缓存的目录做无网络比较。 */
  function paramSource(m) {
  ...
  async function writeModels(provider, models) {
    ...
    try {
      ...
      if (isSettingsConflictError(e)) throw new Error(CONFLICT_MESSAGE)
  ...
  let catalogSnapshot = null          // 最近一次成功聚合结果（供 peekCatalog 使用）

  /**
   * 单源缓存条目上限。
   *
   * 每个条目是一整份解析后的目录（models.dev 快照有上万条），是**大对象**；
   * 用户改几次目录地址就会留下多份。这里按 LRU 语义（最近写入时间）裁剪，
   * 保证常驻内存最多 6 份。
   */
  const CATALOG_CACHE_MAX_ENTRIES = 6
  ...
  async function getCatalog(options) {
    ...
    if (Object.keys(perSource).length) catalogSnapshot = catalog
    return catalog
  }
  ...
  async function discoverModels(args) {
    const baseURL = validateProviderBaseURL(args && args.baseURL)
    ...
    for (const name of Object.keys(customHeadersOf(args && args.headers))) headers[name] = args.headers[name]
  ...
  async function enrichProviderModels(args) {
    ...
    let via = ''
    if (apply && changes.length) via = await writeModels(provider, nextModels)
  ...
  async function deleteModel(args) {
    ...
    if (!isSafeModelId(modelId)) throw new Error('modelId 含非法字符')
    if (!ctx.settings.writable) throw new Error('settings 只读')
    const profile = providerProfile(provider)
    if (!Object.keys(profile).length) throw new Error('渠道不存在: ' + provider)
    const models = getRawModels(provider)
  ...
  async function testModel(args) {
    ...
    if (api === 'openai-completions') {
      ...
      bodyObj = { model: modelId, messages: [{ role: 'user', content: prompt }], stream: false, temperature: 0 }
      bodyObj[maxTokensField] = maxTokens
      if (effort && effort !== 'off' && effortAllowed) {
        if (thinkingFormat === 'openrouter') bodyObj.reasoning = { effort: effort }
        else if (thinkingFormat === 'deepseek') bodyObj.thinking = { type: 'enabled' }
        else bodyObj.reasoning_effort = effort
      }
      if (apiKey) headers.authorization = 'Bearer ' + apiKey
    } else if (api === 'openai-responses') {
      url = joinEndpoint(baseURL, 'responses')
      bodyObj = { model: modelId, input: prompt, max_output_tokens: maxTokens }
      if (effort && effort !== 'off' && effortAllowed) bodyObj.reasoning = { effort: effort }
      if (apiKey) headers.authorization = 'Bearer ' + apiKey
  ...
  async function checkUpdate() {
    ...
    let latestVersion = ''
    let npmUrl = 'https://www.npmjs.com/package/dsh-model-suite'
    try {
      let text = ''
  ...
  async function listModels(args) {
    ...
    return {
      ...
      auto: readAuto(),
      models: getRawModels(provider).map(modelView),
      writable: !!ctx.settings.writable,
    }
  }
      ...
      const patchedResolve = async (provider, model, signal) => {
        ...
        try {
          ...
          const matched = matchModel(model, await getCatalogBounded([model]), {
            baseURL: str(providerProfile(provider).baseURL, ''),
            api: str(providerProfile(provider).api, ''),
          })
