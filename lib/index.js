...
/**
 * inet_aton 语义的紧凑 IPv4 展开（R-低·安全纵深）。
 *
 * `0x7f.0.0.1`、`2130706173`、`127.1` 等数值编码不被点分十进制正则识别，
 * 会被误判为"非 loopback/非元数据地址"——HTTP 面被拦但 **HTTPS 面放行**，
 * 解析器按 inet_aton 语义解析时即绕过 SSRF 防线。这里把它们展开成规范
 * 点分四段；非紧凑形态返回 null（调用方据此停止展开，避免递归）。
 */
function parseIpPartNumber(part) {
  if (/^0x[0-9a-f]{1,8}$/.test(part)) return parseInt(part.slice(2), 16)
  if (/^\d{1,10}$/.test(part)) return parseInt(part, 10)
  return null
}

function expandCompactIpv4(hostname) {
  const value = String(hostname || '').toLowerCase()
  if (!value || !/^[0-9a-fx.]+$/.test(value) || value.indexOf('..') >= 0) return null
  const parts = value.split('.')
  if (parts.length < 1 || parts.length > 4) return null
  const head = []
  for (let i = 0; i < parts.length - 1; i++) {
    const n = parseIpPartNumber(parts[i])
    if (n === null || n > 255) return null
    head.push(n)
  }
  const tail = parseIpPartNumber(parts[parts.length - 1])
  if (tail === null) return null
  const remaining = 4 - head.length
  if (tail > Math.pow(256, remaining) - 1) return null
  const bytes = head.slice()
  let v = tail
  const tailBytes = []
  for (let i = 0; i < remaining; i++) { tailBytes.unshift(v % 256); v = Math.floor(v / 256) }
  return bytes.concat(tailBytes).join('.')
}

function isLoopbackHostname(hostname) {
  let value = String(hostname || '').toLowerCase().replace(/^\[|\]$/g, '')
  // 尾点 FQDN（localhost. / 127.0.0.1.）是合法写法，剥掉再判
  ...
  const expanded = expandCompactIpv4(value)
  if (expanded && expanded !== value) return isLoopbackHostname(expanded)
  return false
}
...
function isBlockedOutboundHostname(hostname) {
  let value = String(hostname || '').toLowerCase().replace(/^\[|\]$/g, '')
  // 尾点 FQDN 剥掉再判；数值编码 IP（R-低）展开成规范点分四段后按段判定
  while (value.endsWith('.')) value = value.slice(0, -1)
...
/**
 * **保留变体后缀**的归一化（R-M4 第 1 级前置键）：lower → 去 query/fragment →
 * 取最后一段 path，但**不剥** `:tag`/`@tag`。
 *
 * normalizeModelIdKey 的 tag 剥剥离会让 `claude-sonnet-4-5@20250929` 与基名/
 * 其它快照合入同一桶，"精确命中"返回整桶多变体后可能选错变体的元数据。
 * byRaw 索引先按带 tag 的完整键精确命中，命中失败才回落到剥 tag 的 byNorm。
 */
function normalizeModelIdRaw(id) {
  let s = String(id || '').trim().toLowerCase()
  if (!s) return ''
  s = s.split('?')[0].split('#')[0]
  if (s.indexOf('/') >= 0) {
...
function validateCompatValueFor(field, type, values, value) {
  ...
  if (type === 'object') {
    ...
    for (const key of Object.keys(value)) {
      // ★ R-M3：同 headers——原型键名的赋值会改写 out 的原型而非建自有键，
      //   事后 Object.keys(out).length === 0 会被误判为"未设置/显式清空"。
      if (key === '__proto__' || key === 'prototype' || key === 'constructor') {
        throw new Error('compat.' + field + ' 的键名非法（原型键名）: ' + key)
      }
      out[key] = normalizeChatTemplateValue(value[key])
    }
    return out
  }
...
function normalizeHeadersInput(raw) {
  ...
  for (const name of Object.keys(raw)) {
    ...
    // ★ R-M3：__proto__ 等原型键名通过字符集校验后会被赋值语义吞掉（不建自有键），
    //   唯一键时被解读为"显式清空 headers"而删除已存配置——必须显式拒绝。
    if (key === '__proto__' || key === 'prototype' || key === 'constructor') {
      throw new Error('请求头名称非法（原型键名）: ' + clipText(key, 60))
    }
...
function sanitizeDiagnosticText(value, max) {
  return clipText(value, max)
    // ★ R-低：api[-_]key 下划线形态、全角冒号「：」、常见厂牌裸键前缀一并脱敏
    .replace(/(authorization|proxy-authorization|x-api-key|api[-_]?key|token|bearer)\s*([:=：]\s*|\s+)[^\s,;"']+/gi, '$1: [redacted]')
    .replace(/\b(?:sk-|xai-|gsk_|AIza|r8_|tgrk_)[A-Za-z0-9._-]{6,}\b/g, '[redacted-key]')
}
...
/** SVG 语义还原：数值/命名实体解码后再做黑名单扫描（R-低：`jav&#x09;ascript:` 绕过）。 */
...
function extractSvgMarkup(text) {
  ...
  // ★ R-低·纵深防御：`<script` 不再要求后随空白/>（`<script/src=` 绕过）；
  //   实体解码后再扫 javascript:/on*；SMIL `<set attributeName="on…">` 可把
  //   事件属性"种"到外层元素，一并拒绝；data: 仅放行位图图片。
  //   客户端另有 sandbox:'' iframe 兜底，这里是双层防线的前一层。
  const decoded = decodeSvgEntities(svg)
...
function makeSuiteMeta(id, provider, source, fields) {
  ...
  const meta = {
    ...
    rawId: normalizeModelIdRaw(id),
    looseId: normalizeModelIdKey(id).replace(/[._]+/g, '-'),
...
/** 元数据指纹（用于多候选择一的指纹比较）。按对象记忆化——多候选匹配热路径
 *  （每次保存/resolveModelInfo 对每个候选重复调用）免去重复 JSON.stringify。 */
function metadataFingerprint(meta) {
  if (meta && typeof meta === 'object' && typeof meta.__fp === 'string') return meta.__fp
  ...
  if (meta && typeof meta === 'object') {
    try { Object.defineProperty(meta, '__fp', { value: fp, enumerable: false, writable: true, configurable: true }) } catch (_) {}
  }
  return fp
}
...
/** 主机名是否含某 provider 标签（R-低：子串匹配会把 maxai.example.com 判给 xai）。 */
function hostnameHasProviderLabel(hostname, label) {
  if (!hostname || !label) return false
  return String(hostname).split('.').indexOf(label) >= 0
}
...
function providerPreferScore(provider, modelId, hostname) {
  ...
  if (hostnameHasProviderLabel(hostname, pid)) score += 50
...
function pickCatalogCandidate(candidates, id, baseURL, api) {
  ...
  // 1) 主机名命中（按域名标签，不做子串匹配）
  const hostMatch = candidates.find((meta) => meta.provider && hostnameHasProviderLabel(hostname, meta.provider))
...
/**
 * 两级匹配（§8.5，❌ 无前缀模糊匹配）。
 * 第 0 级 = byRaw：保留 `:tag/@tag` 的完整键精确命中（R-M4）——多变体同基名时
 *             优先取用户写的那个变体，避免按文件序选错快照；
 * 第 1 级 = 规范化相等（byNorm，剥 tag）；第 2 级 = 宽松相等（byLoose，分隔符等价）。
 */
function matchModelCandidates(id, catalog) {
  if (!catalog || !catalog.byNorm) return { candidates: [], level: 0 }
  const raw = normalizeModelIdRaw(id)
  if (raw && catalog.byRaw) {
    const rawHit = catalog.byRaw.get(raw)
    if (rawHit && rawHit.length) return { candidates: rawHit.slice(), level: 1 }
  }
...
function mergeLayered(perSource) {
  ...
  for (const normId of allNormIds) {
    ...
    for (const meta of merged) {
      ...
      // R-M4：带 tag 的完整键单独建索引——同基名的多变体（@快照/:free/:extended）
      // 在 byNorm 里合桶，靠 byRaw 才能按"用户写的完整 id"精确命中。
      const raw = meta.rawId || meta.id
      if (raw) {
        const rawBucket = byRaw.get(raw)
        if (rawBucket) rawBucket.push(meta)
        else byRaw.set(raw, [meta])
      }
    }
  }
  return { byNorm: byNorm, byLoose: byLoose, byRaw: byRaw }
}
...
function enrichOpValue(value, catalog, auto, route, fields) {
  ...
  if (Array.isArray(value)) {
    // ★ R-低：删掉 `value.some(looksLikeModelEntry)` 兜底——它与 R11"只在 models
    //   键路径富化"的修正矛盾：任意带 id 元素的数组（如未来 schema 的非模型
    //   数组字段）都会被注入容量/推理档。op 路径已由 tail/inModelsArray 覆盖。
    const isModelsArray = tail === 'models' || inModelsArray
...
export function apply(ctx) {
  let hostProto = null
  /* ★ R-优化：describe() 结果微缓存。一次保存链路会调 describe 3-4 次
   * （refreshHostProto / getUserLayer / readNsRevision / listProviders），
   * 每次都是全命名空间 base+user 的深克隆。150ms TTL 把它们合并成一次；
   * 本插件自身的每次成功写入都会显式失效，避免连续保存读到过期 revision
   * 造成假 409（外部进程在窗口内的并发写入仍由 CAS 409 语义兜底）。 */
  const DESCRIBE_CACHE_TTL_MS = 150
  let describeCache = { at: 0, list: null }
  function describeList() {
    const now = Date.now()
    if (describeCache.list && (now - describeCache.at) < DESCRIBE_CACHE_TTL_MS) return describeCache.list
    try {
      const list = ctx.settings.describe()
      if (Array.isArray(list) && list.length) {
        describeCache = { at: now, list: list }
        return list
      }
    } catch (_) {}
    return describeCache.list
  }
  function invalidateDescribe() { describeCache = { at: 0, list: null } }
  ...
  function H(data) {
    ...
    if (data && typeof data === 'object') {
      ...
      // ★ R-M3：defineProperty 组装——`o[k] = v` 在 k 为 '__proto__' 时是改原型
      //   而非建自有键（该子树随后序列化蒸发）；hostProto 链上若有访问器属性，
      //   赋值还会走 setter 造成静默丢字段。defineProperty 两种语义都绕开。
      for (const k of Object.keys(data)) {
        if (data[k] === undefined) continue
        Object.defineProperty(o, k, { value: H(data[k]), enumerable: true, writable: true, configurable: true })
      }
      return o
    }
    return data
   ...
   *  （R-低：从纯函数区段移入闭包——setTimeout 定时器副作用与 smoke 的
   *  "纯函数 + 只依赖模块常量"区段契约不符。） */
    ...
    return new Promise((resolve, reject) => {
      Promise.all([import('node:https'), import('node:http')]).then(([httpsMod, httpMod]) => {
        ...
        const doRequest = (reqOpts) => {
          ...
          // ★ R-优化：外部取消（如浏览器断开测试页）——10 分钟的测试请求不再白跑
          if (opts.signal && typeof opts.signal.addEventListener === 'function') {
            opts.signal.addEventListener('abort', () => {
              req.destroy(new Error('客户端已断开，请求中止'))
            }, { once: true })
          }
        ...
        // ★ R-低：代理自身是 https:// 时 CONNECT 也要走 TLS——旧实现固定用
        //   httpMod（明文连 TLS 代理端口），HTTPS 代理配置必挂。
        const tunnelLib = proxy.protocol === 'https:' ? httpsMod : httpMod
        const tunnelReq = tunnelLib.request(Object.assign({
  ...
  async function getCatalog(options) {
    ...
    // ★ R-优化：modelsDev 先行（命中即省流量），需要补缺时另两源**并行**拉取——
    //   旧实现三源串行，enrich 冷路径最坏 20+15+15s。
    if (enabled.indexOf('modelsDev') >= 0) {
      const sourceId = 'modelsDev'
    ...
    if (needMore) {
      const rest = enabled.filter((id) => id !== 'modelsDev')
      if (rest.length) {
        const results = await Promise.all(rest.map(async (sourceId) => {
          const spec = cfg[sourceId]
          try {
            const metas = await fetchSource(sourceId, spec.url, !!opts.force)
            return { sourceId: sourceId, metas: metas }
          } catch (e) {
            const message = e && e.message ? e.message : String(e)
            return { sourceId: sourceId, error: clipText(message, 200) }
          }
        }))
        for (const r of results) {
          if (r.error) {
            sourceErrors[r.sourceId] = r.error
            catalogErrors.set(r.sourceId, r.error)
            perSource[r.sourceId] = []
          } else {
            perSource[r.sourceId] = r.metas
            sourcesUsed.push(CATALOG_SOURCE_LABEL[r.sourceId])
            catalogErrors.delete(r.sourceId)
          }
        }
      }
    }
  ...
  async function addProviderModels(args) {
    ...
    const incoming = normalizeCreateModels(args && args.models, str(profile.api, '').trim())
  ...
  function normalizeCreateModels(raw, api) {
    if (!Array.isArray(raw) || !raw.length) throw new Error('至少需要一个模型')
    // ★ R-F6：compat 校验沿用 save 路径的**当前协议** offer 表（空 api = 未知协议
    //   → 空表 → 非空 compat 报错），不再按全协议并集放行——跨协议字段会通过
    //   add 落盘后被平台 resolveModelCompat 判 invalid，三通道全失败报纠缠错误。
    const compatOffer = COMPAT_OFFER[api]
    const out = []
    const seen = Object.create(null)
    for (let i = 0; i < raw.length; i++) {
      ...
      if (isPlainObject(entry)) {
        ...
        // ★ R-F6：reasoningEfforts 非法输入**显式报错**（与 normalizeEditor 同口径），
        //   不再交给 cloneModel→sanitizeEfforts 静默丢键（"保存成功"但其实没存上）。
        if (entry.reasoningEfforts !== undefined) {
          if (entry.reasoningEfforts === false) {
            m.reasoningEfforts = false
          } else if (isPlainObject(entry.reasoningEfforts)) {
            const efforts = sanitizeEfforts(entry.reasoningEfforts)
            if (!efforts) throw new Error('模型 ' + (i + 1) + ' 的 reasoningEfforts 需要至少一个非 off 档（要关闭推理请传 false）')
            m.reasoningEfforts = efforts
          } else {
            throw new Error('模型 ' + (i + 1) + ' 的 reasoningEfforts 需要对象或 false')
          }
        }
        if (isPlainObject(entry.compat)) {
          // normalizeCompatInput 自带口径：未知协议+空对象=不动（B5）；不支持的字段=报错
          const compatNorm = normalizeCompatInput(entry.compat, api)
          if (compatNorm.present && compatNorm.value !== undefined) m.compat = compatNorm.value
        }
      }
  ...
  async function enrichProviderModels(args) {
    ...
    let message = ''
    if (changes.length) {
      message = (applyNow ? '已从目录写回 ' : '可从目录补全 ') + changes.length + ' 个模型（命中 ' + hitCount + '/' + existing.length + '）'
    } else if (hitCount) {
  ...
  function normalizeEditor(editor, api) {
    ...
    for (const row of (Array.isArray(editor.levels) ? editor.levels : [])) {
      ...
      if (level === 'off') {
        // ★ off 的 wire 三态：空串 /「空传」→ null（选 off 时**不发送**强度参数）；
        //   填了值 → 原样落盘（pi-ai：`off` with a value sends that value）。
        //   默认开思考的网关要"选 off 时显式关闭"（如 reasoning_effort: none）
        //   只有这条路可走——此前硬编码 efforts.off = null 会把 UI 里填的值
        //   静默丢弃，保存重载后又变回「空传」。
        const offWire = str(row.wire, '').trim()
        if (offWire.length > 64) throw new Error('off 的 wire 值过长')
        efforts.off = offWire || null
      } else {
  ...
  async function saveSources(args) {
    ...
    if (isPlainObject(incoming)) {
      for (const key of Object.keys(incoming)) {
        ...
        // ★ R-M6：地址为空 ⇒ 强制未启用，**且不受后续 enabled:true 影响**——
        //   旧实现先置 enabled=false 再被 entry.enabled=true 覆盖回去，
        //   "清空地址保存"实际存成"启用 + 沿用旧地址"，与 UI 提示
        //   「地址为空视为未启用」直接矛盾。
        let urlEmpty = false
        ...
        if (Object.prototype.hasOwnProperty.call(entry, 'enabled')) {
          ...
          if (!urlEmpty) next[key].enabled = entry.enabled
        }
      }
    }
    ...
    // ★ R-F7：失败冷却也一并复位——慢失败后 60s 冷却窗口内用户换了可用地址，
    //   旧实现仍按冷却返回空目录，与"立刻生效"的意图相悖。
  ...
  /** 真调一次模型 API。signal：客户端断开时中止上游请求。 */
  async function testModel(args, signal) {
    const provider = str(args && args.provider, '').trim()
    const modelId = str(args && args.modelId, '').trim()
    if (!provider) throw new Error('缺少 provider')
    ...
    // ★ R-F4：wire 值映射——pi-ai dispatch 一律发 `thinkingLevelMap[effort] ?? effort`
    //   （用户自定 wire 如 high→'max_effort'），测试此前发原始档位名，与真实会话
    //   不一致。off 档配了 wire 值（R-F1 能力）时同样发送该值而非省略。
    const effortsMap = modelEntry && isPlainObject(modelEntry.reasoningEfforts) ? modelEntry.reasoningEfforts : null
    const levelWire = (level) => {
      if (!effortsMap) return level // 未配置档位表：identity（与 map ?? effort 等价）
      if (!Object.prototype.hasOwnProperty.call(effortsMap, level)) return null // 未声明档：dispatch 会拒绝
      const w = effortsMap[level]
      return w === null || w === undefined || w === '' ? null : String(w)
    }
    const effortWire = (effort && effort !== 'off' && effortAllowed) ? levelWire(effort) : ''
    // off 请求且模型为 off 配了 wire 值 → 发送该值（pi-ai 731-735 行同语义）
    const offWire = (effort === 'off' && effortAllowed) ? levelWire('off') : ''
    const effortRequested = !!effortWire
    ...
    if (api === 'openai-completions') {
      ...
      if (effortRequested) {
        ...
        if (thinkingFormat === 'openrouter' || thinkingFormat === 'ant-ling') {
          bodyObj.reasoning = { effort: effortWire }
          effortApplied = true
        } else if (thinkingFormat === 'deepseek') {
          bodyObj.thinking = { type: 'enabled' }
          if (effortSubParamAllowed) bodyObj.reasoning_effort = effortWire
          effortApplied = true
          ...
          bodyObj.thinking = effortWire
          effortApplied = true
        } else if (thinkingFormat === 'chat-template' || thinkingFormat === 'baseten') {
          // 依赖用户在 chatTemplateKwargs/Args 里自配的 $var 展开，测试无法可靠构造
          effortApplied = false
        } else {
          bodyObj.reasoning_effort = effortWire
          effortApplied = true
        }
      } else if (offWire) {
        // off 档带 wire 值：按 pi-ai 731-735 行同语义把显式关闭值发上 wire
        if (thinkingFormat === 'openrouter' || thinkingFormat === 'ant-ling') {
          bodyObj.reasoning = { effort: offWire }
          effortApplied = true
        } else if (thinkingFormat === 'deepseek' || thinkingFormat === 'string-thinking') {
          bodyObj.thinking = offWire
          effortApplied = true
        } else {
          bodyObj.reasoning_effort = offWire
          effortApplied = true
        }
      }
      if (apiKey) headers.authorization = 'Bearer ' + apiKey
    ...
    return {
      ...
      hasApiKey: !!apiKey,
      source: source,
      effortApplied: (effortRequested || !!offWire) ? effortApplied : undefined,
  ...
  /** check-update 结果缓存（R-F5）：GET 端点无频控，本机任意页面可高频触发；
   *  10 分钟内直接复用上次结果，避免把查询频率暴露给 npm registry。 */
  const UPDATE_CACHE_TTL_MS = 10 * 60 * 1000
  let updateCache = null // { at, result }

  async function checkUpdate() {
    if (updateCache && (Date.now() - updateCache.at) < UPDATE_CACHE_TTL_MS) return updateCache.result
    ...
    updateCache = { at: Date.now(), result: result }
    return result
  }
  ...
  function installPatches() {
    ...
    const patchSettings = (key, buildValue) => {
      ...
      const patchedMethod = async (ns, input, expectedRevision) => {
        // ★ R-F3：闸门段（readAuto/collectModelIds）自身异常时降级为"补丁不存在"——
        //   三链路的统一承诺是原调用必须可达；但 bound 调用的失败语义保持原样
        //   （非冲突失败仅在"富化改写过入参"时用原始入参重试一次，不额外重试）。
        let gate = null
        try {
          // ① 命名空间闸门（修正 enhancer R1：原实现对所有 ns 一视同仁）
          const auto = readAuto()
          if (ns !== NS || !auto.enabled || !auto.persistOnSave) gate = null
          else {
            // 没有模型条目（例如写插件偏好本身）→ 无富化可言，既不联网也不改写
            const ids = collectModelIds(input, key)
            if (ids.length) gate = { auto: auto, ids: ids }
          }
        } catch (_) { gate = null }
        if (!gate) return bound(ns, input, expectedRevision)
        let next = input
        try {
          const catalog = await getCatalogBounded(gate.ids)
          next = buildValue(input, catalog, gate.auto)
        } catch (_) {
          next = input
        }
        try {
    ...
    if (llm && typeof llm.resolveModelInfo === 'function') {
      ...
      const patchedResolve = async (provider, model, signal) => {
        ...
        try {
          ...
          // ★ R-M2：显式关闭推理的模型（reasoningEfforts === false）**不注入**目录
          //   档位——pi-ai 对 false 不产出 info.reasoning，而 dispatch 侧对非推理
          //   模型只支持 off 档（resolveReasoningLevel 对其它档抛
          //   UNSUPPORTED_REASONING_EFFORT）。旧实现只看 !info.reasoning，无法
          //   区分"未配置"与"显式关闭"，注入后能力面与请求面打架。
          if (Array.isArray(routeProfile.models)) {
            const saved = routeProfile.models.find((m) => m && sameModelId(m.id, model))
            if (saved && saved.reasoningEfforts === false) return info
          }
  ...
  async function calibrateCatalogRoutes() {
    // ★ R-M10：数据源修正——真机 `getBuiltinProviders()` 返回的是**裁剪后**的
    //   id 列表（实测 39，无 radius），而平台 catalogProviders() 判定用的是
    //   `builtinProviders()`（完整 40，对象数组）。两者都试，优先与平台判定
    //   同源的对象形态；`@deepseek-ai/dsh-llm-pi-ai` 主入口不 re-export 这两个
    //   函数，保留它仅作为未来版本可能补导出的兜底。
    for (const spec of ['@earendil-works/pi-ai/providers/all', '@deepseek-ai/dsh-llm-pi-ai']) {
      try {
        const mod = await import(spec)
        let ids = null
        if (mod && typeof mod.builtinProviders === 'function') {
          const providers = mod.builtinProviders()
          if (Array.isArray(providers) && providers.length) ids = providers.map((p) => (p && typeof p === 'object' ? p.id : p))
        }
        if ((!ids || !ids.length) && mod && typeof mod.getBuiltinProviders === 'function') {
          ids = mod.getBuiltinProviders()
        }
  ...
  function postRoute(path, run) {
    return {
      ...
      handler: (req, res) => {
        ...
        // ★ R-优化：客户端在长操作（test-model 最长 10 分钟）期间断开时中止上游请求
        ...
        readJsonBody(req).then(
          (body) => {
            ...
            return run(record, clientAbort.signal).then(
              (value) => json(res, 200, value),
  ...
  function makeRoutes() {
    return [
      ...
      postRoute(`${API_PREFIX}/test-model`, (b, signal) => testModel(b, signal)),
