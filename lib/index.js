...
export function apply(ctx) {
  ...
  async function discoverModels(args) {
    ...
    const api = str(args && args.api, hasProfile ? str(profile.api, 'openai-completions') : 'openai-completions').trim() || 'openai-completions'
    ...
    if (norm.compatPresent) {
      if (norm.compat === undefined) delete next.compat
        ...
        for (const key of Object.keys(baseCompat)) {
          if (COMPAT_ALL_FIELDS[key] || compat[key] !== undefined) continue
